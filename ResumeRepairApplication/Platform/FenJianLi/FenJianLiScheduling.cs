    ...
    public class FenJianLiScheduling 
    {
        ...
        public static void Start(SchedulingShowForm _ssf, CheckNumberForm _cnf)
        {
            ...
            new Thread(() =>
            {
                ...
                {
                    new Tuple<Func<DataResult>, int, string, bool>(new RegisterSpider().Init, 3 * 1000, "注册", false),
                    new Tuple<Func<DataResult>, int, string, bool>(new ActivationSpider().Init, 5 * 1000, "激活", false),
                    new Tuple<Func<DataResult>, int, string, bool>(new LoginSpider().Init, 3 * 1000, "签到",false),
        ...
        public void DoWork()
        {
            this._workTimer = new System.Timers.Timer();
            this._workTimer.Interval = 1000;
            this._workTimer.AutoReset = false;
            this._workTimer.Elapsed += new System.Timers.ElapsedEventHandler(this.TimerHanlder);
            if(this._isInitStart) this._workTimer.Start(); // 初始化执行
        }
        ...
        private void TimerHanlder(object sender, System.Timers.ElapsedEventArgs e)
        {
            lock (this._workTimer)
            {
                ...
                do
                {
                    try
                    {
                        dataResult = this._taskAction.Invoke();
                    }
                    catch (Exception ex)
                    {
                        dataResult.IsSuccess = false;

                        dataResult.ErrorMsg = string.Format("{5}纷简历 平台{0}定时任务程序异常！{1}异常消息：{2}{3} 堆栈异常信息：{4}", this._taskName, Environment.NewLine, ex.Message, Environment.NewLine, ex.StackTrace, Environment.NewLine);
                    }
                ...
                this._workTimer.Interval = this._interval;
